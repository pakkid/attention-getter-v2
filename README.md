...
- **Dual-boot PCs**: pair each OS under the same PC name, or pair them separately and use **Merge** on one of them. They then show up as one PC with an install per OS: whichever OS is running is the one that's online and gets the alerts. History, groups and trigger keys move over in a merge. On a merged PC, **Split off** makes an install its own PC again and **Remove** revokes just that OS's pairing. Clients from 1.1.0 report their OS and version, which are shown next to each install.
