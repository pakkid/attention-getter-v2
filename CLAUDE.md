...
- **PCs and installs:** a PC (`devices`) can have several installs (`installs`), each with its own token, e.g. both OSes of a dual-boot PC. Device auth resolves a token to its install, the hub keys connections by install id, and alerts go to every connected install of the PC. Pairing under an existing PC name adds an install instead of creating a PC.
...
- **Caching:** `staticHandler` in `server/internal/api/api.go` serves every web file with `Cache-Control: no-cache` plus a content-hash ETag. Keep it that way. Cloudflare still rewrites that to a 4-hour browser TTL on `.js`/`.css` (its Browser Cache TTL setting), so the handler also replaces each `?v=` in `index.html` (which isn't cached) with that file's content hash. A deploy that changes a file changes its URL; never bump `?v=` by hand, and link any new same-origin script or stylesheet with `?v=` too. `sw.js` handles push only and caches nothing.
